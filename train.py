...
DATADIR_VALID = 'dataset_train/valid'
NUM_EPOCHS = 5000
...
def main(argv=None):
    with tf.Graph().as_default():
        ...
        print('Loaded %d * %d * %d datas.(totals: %d)' % (batch_len, epoch_size, FRAME_COUNT, train_size))
        ...
        x = tf.placeholder(tf.float32, shape=[BATCH_SIZE, FRAME_COUNT, 9], name='data')
        y = tf.placeholder(tf.float32, shape=[BATCH_SIZE, FRAME_COUNT, 3])
        ...
        with tf.Session() as sess:
            ...
            for step in range(NUM_EPOCHS * epoch_size):
                ...
                if offset == 0:
                    state = sess.run(initial_state)
                batch_data = np.reshape(data[:, offset, :, :], [BATCH_SIZE, FRAME_COUNT, 9])
                batch_labels = np.reshape(label[:, offset, :], [BATCH_SIZE, FRAME_COUNT, 3])
                ...
                if step % EVAL_FREQUENCY == 0:
                    elapsed_time = time.time() - start_time
                    start_time = time.time()
                    print('Step %d (epoch %f), %f ms' %(step, np.float32(step) / epoch_size,
